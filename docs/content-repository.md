...
MikanArchive 使用公开框架仓库和私有内容仓库分离的方式维护内容。公开仓库保存主题代码、页面、组件、样式、脚本、示例内容和文档；私有内容仓库保存真实文章、图片、资源收藏、友链、个人资料、简历配置和可选时间线记录。
...
当前公开站点不再提供独立足迹功能页，以下 `records/` 数据只作为可同步的历史记录数据源保留，供后续重新接入页面、小组件或私有扩展时使用。
...
`profile/about.md` 保存“个人介绍”页面中的长文本介绍。
