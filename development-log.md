...
- 为公开日历补充暗色模式样式契约测试，锁定月份标题、日期数字、事件说明文字和蓝色事件卡片的暗色覆盖规则。
- 暗色模式下将日历月份标题、日期数字和农历标签改为更高可读性的浅色文字。
- 将暗色模式事件卡片从亮色浅蓝背景调整为低饱和半透明色块，并为蓝、粉、薄荷、黄和中性色事件分别设置暗色文字与背景。
- 修复事件卡片内说明文字在暗色模式下被通用 meta 颜色覆盖、导致浅色事件背景上文字不清的问题。
- 同步 `docs/next-tasks.md` 和 CHANGELOG，记录本次暗色可读性修复。

### 验证情况

- 已运行 `npm.cmd run test:pages`，页面契约测试通过。
- 已使用系统 Chrome + Playwright 强制暗色模式检查 `http://127.0.0.1:4321/calendar/`：月份标题颜色为 `rgba(255, 255, 255, 0.9)`，日期数字颜色为 `rgba(255, 255, 255, 0.84)`，蓝色事件卡片背景为 26% 透明低饱和蓝，说明文字颜色为 `rgba(255, 255, 255, 0.74)`，控制台无错误。
- 已生成暗色截图 `output/playwright/calendar-dark-fix.png` 用于本地视觉确认；`output/` 已被 `.gitignore` 忽略，不会提交。

## 2026-06-29 21:27:23 +08:00
...
- 公开日历功能已完成实现、验证和记录，按交接规则清空 `docs/next-tasks.md` 中的当前目标内容，仅保留文档使用规则。

### 验证情况

- 本次仅清理交接文档并补充日志，未改动应用代码；公开日历的完整验证已记录在上一条开发日志中。
